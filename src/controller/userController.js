...
class UserController {
  static async getBandUser(req, res) {
    try {
      const user = await Models.user.find({ isPubilc: false });
      if(!user){
        return SendError(res,404,statusMessage.NOT_FOUND+" user");
      }
      return SendSuccess(res,SMessage.SUCCESS,user);
    } catch (error) {
      return SendError(res, 500, statusMessage.SERVER_ERROR, error);
    }
  }
  static async bandUser(req, res) {
    try {
